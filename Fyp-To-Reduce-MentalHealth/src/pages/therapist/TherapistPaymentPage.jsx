...
import { Bell, ChevronRight, Eye, Search } from 'lucide-react'
  ...
  const updatePaymentStatus = async (id, paymentStatus) => {
    ...
    try {
      const json = await api(`/appointments/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ status: 'CONFIRMED', paymentStatus }),
      })
            ...
            <div className="rounded-[30px] border border-black/5 bg-cream p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-3 text-sm text-[#556b5b]">All payment records</p>
                </div>
                <div className="rounded-full bg-[#e8f3ea] px-3 py-1 text-[#1f5f4a] font-semibold">{total}</div>
              </div>
              ...
              {loading && <p className="mt-4 text-sm text-[#66746b]">Loading payments…</p>}
                      ...
                      <tr key={item.id} className="text-sm text-[#3f4f41]">
                        ...
                        <td className="px-3 py-4 border border-black/10">
                          <select
                            ...
                            className={`rounded-full px-3 py-2 text-[12px] font-semibold outline-none border ${paymentStyles[item.paymentStatus] || paymentStyles.PENDING}`}
                          >
