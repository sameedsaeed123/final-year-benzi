...
import { evaluateAiScopeGuard } from '../services/aiScopeGuardService.js'
import {
...
function mapGoalForResponse(g, viewerRole) {
  ...
  if (g.crisisFlag && viewerRole === 'patient') {
    return {
      ...base,
      title: 'Safety alert',
      description: 'Your therapist has been notified. Please message them in the app.',
      isCrisisAlert: true,
...
export async function patientAiChat(req, res, next) {
  try {
    ...
    if (crisis.isCrisis) {
      const alert = await notifyTherapistOfCrisis(patientUserId, crisis, 'benzi_ai_chat')
      if (!alert.notified) {
        console.warn('[AI Chat] Crisis detected but therapist alert failed:', alert.reason || alert.error)
      }
    }
    ...
    if (crisis.isCrisis) {
      ...
      const scopeGuard = evaluateAiScopeGuard(text, context)
      if (scopeGuard.refuse) {
        aiResponse = scopeGuard.reply
      } else {
