...
export default function TherapistSidebar({ activeItem = 'Dashboard' }) {
	...
	const content = (onNavigate) => (
		<>
			<div className="px-4 sm:px-5 pt-6 pb-2">
			...
			<div className="flex items-center justify-between gap-2 px-4 sm:px-5 pt-4 pb-4">
				<span className="text-[13px] uppercase tracking-[0.18em] font-semibold text-white/80 shrink-0">Menu</span>
				...
				{items.map((item) => {
					...
					return (
						<Link
							...
							to={item.to}
							onClick={onNavigate}
							className={`w-full flex items-center gap-2.5 px-3 py-3 text-left transition-all rounded-full text-[14px] ${isActive ? 'bg-white text-brand font-semibold' : 'hover:bg-white/10'}`}
						>
	...
	return (
		<>
			...
			<aside className="hidden xl:block bg-brand text-white w-full rounded-[30px] overflow-visible xl:sticky xl:top-[4.75rem] self-start">
				{content(null)}
			</aside>
