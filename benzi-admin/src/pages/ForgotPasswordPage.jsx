    ...
    <div className="grid grid-cols-6 gap-3">
      {value.map((digit, index) => (
        <input
          ...
          className="w-full h-14 text-center border border-black/10 rounded-xl text-[20px] font-semibold bg-[#f5f5f5] focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none"
        />
      ))}
    </div>
  )
          ...
          <div className="w-full max-w-xl bg-white rounded-[30px] shadow-sm border border-black/5 px-10 py-12 max-[640px]:px-6 max-[640px]:py-8">
            ...
            {step === 1 && (
              <form onSubmit={handleSendCode} className="flex flex-col gap-5">
                ...
                <button
                  type="submit"
                  className="w-full bg-brand text-white py-4 rounded-2xl text-[15px] font-semibold transition-all hover:bg-brand-dark hover:-translate-y-px"
                >
                  Send Verification Code <ArrowRight size={16} className="inline-block ml-2" />
                </button>
            ...
            {step === 3 && (
              <form onSubmit={handleResetPassword} className="flex flex-col gap-5">
                ...
                >
                  Reset Password <ArrowRight size={16} className="inline-block ml-2" />
                </button>
              </form>
            )}
          </div>
